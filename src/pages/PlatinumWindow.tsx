...
import { PRODUCTS, PROVENANCE } from '../constants';

const PW_PRODUCT = PRODUCTS.find(p => p.title === 'Platinum Window')!;
  ...
  return (
    <div className="font-sans bg-slate-50 text-slate-800 p-0 m-0 overflow-x-hidden min-h-screen pb-12">
      <style>{`
        ...
        @media (max-width: 768px) { .pw-hero-inner { grid-template-columns: 1fr; } }
        .pw-hero-eyebrow {
          display: inline-flex;
          align-items: center;
          gap: 0.4rem;
        ...
        .pw-hero-actions { display: flex; gap: 0.75rem; flex-wrap: wrap; }
        .pw-btn-primary {
          display: inline-flex;
          align-items: center;
          gap: 0.4rem;
        ...
        .pw-btn-primary:hover { background: #6d28d9; transform: translateY(-1px); color: white; }
        .pw-dismiss {
          display: inline-flex;
          align-items: center;
          gap: 0.4rem;
        ...
        .pw-dismiss:hover { background: rgba(255,255,255,0.14); color: white; }
      `}</style>
          ...
          <div className="pw-hero-inner">
            <div>
              <div className="pw-hero-eyebrow">
                ...
                LIVE · Ovarian Cancer · HGSOC
              </div>
              <h1 className="pw-hero-title">Platinum Window</h1>
              <p className="pw-hero-sub">
                Patient-specific molecular countdown before the stromal cage closes. Replace 21-day calendar dosing with a data-driven treatment window from 10-gene tumor profiling.
              </p>
              <div className="pw-hero-stats">
                {PW_PRODUCT.stats?.map(s => (
                  <div key={s.label} className="pw-hero-stat">
                    <span className="pw-hero-stat-val">{s.value}</span>
                    <span className="pw-hero-stat-lbl">{s.label}</span>
                  </div>
                ))}
              </div>
              <div className="pw-hero-actions">
                <button className="pw-btn-primary" onClick={() => { setActiveTab('engine'); setShowHero(false); }}>
                  Score a Patient <ArrowRight size={13} />
                </button>
                <button className="pw-dismiss" onClick={() => setShowHero(false)}>
                  Skip intro
                </button>
              </div>
            </div>

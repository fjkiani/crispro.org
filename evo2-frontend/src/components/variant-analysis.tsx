...
import { analyzeVariantWithAPI, type AnalysisResult, type ClinvarVariant, type GeneBounds, type GeneFromSearch } from "~/utils/genome-api"
...
const VariantAnalysis = forwardRef<VariantAnalysisHandle, VariantAnalysisProps>(({
    ...
    const handleVariantSubmit = async (pos: string, alt: string) => {
        ...
        if (!validNucleotides.test(alt)) {
            setVariantError("Nucleotide must be A, C, G, or T")
            return
        }
        ...
        try {
            ...
            // console.log(1)
            // console.log(variantResult?.delta_score)
            // console.log(2)
            // console.log(variantResult?.splice_risk)
        } catch (err) {
            console.error(err)
            ...
            <CardContent className="pb-4">
                ...
                <div className="flex flex-wrap items-end gap-4">
                    <div>
                        ...
                        <Input
                            ...
                            className="h-8 w-32 border-[#3d4f3d]/10 test-xs"
                        >
                    ...
                    <Button 
                        ...
                        className="h-8 cursor-pointer bg-[#3c4f3d] text-xs text-white hover:bg-[#3c4f3d]"
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <div className="flex items-center justify-end">
                                    <Button 
                                        ...
                                        variant="outline"
                                        size="sm"
                                        className="h-7 cursor-pointer bg-[#e9eeea] text-xs text-[#3c4f3d] border-[#3c4f3d]/20 hover:bg-[#3c4f3d]/10"
                                        onClick={() => 
                                            {
                ...
                {variantResult && (
                    <div className="mt-6 rounded-md border border-[#3c4f3d]/10 bg-[#e9eeea]/30 p-4">
                        <h4 className="mb-3 test-sm font-medium text-[#3c4f3d]">
                            Analysis Results
                        </h4>
                        <div className="grid gap-4 md:grid-cols-2">
                            <div>
                                <div className="mb-2">
                                    ...
                                    <div className="text-sm">
                                        {gene?.symbol} {variantResult.position.toString()}{" "}
                                        <span className="font-mono">
                            ...
                            <div>
                                ...
                                <div className="mt-3">
                                    ...
                                    <div className="mt-1 h-2 w-full rounded-full bg=[#e9eea]">
                                        <div 
                                ...
                                <div className={`inline-block rounded-lg px-3 py-1 text-xs ${getRiskColorClasses(variantResult.splice_risk)}`}>
                                    {variantResult.splice_risk}
                                </div>
                                {variantResult.splice_position && variantResult.splice_boundary && (
                                    <div className="text-xs text-[#3c4f3d]/60">
                                        Variant is {variantResult.splice_position == variantResult.position
                                            ? `at the ${variantResult.splice_boundary}`
                                            : `${Math.abs(variantResult.splice_position - variantResult.position)} bp 
                                            ${variantResult.splice_position < variantResult.position ? "downstream":"upstream"}   
                                            from ${variantResult.splice_boundary} `
                                        } of an exonic region
                                    </div>
                                )}

...
import { 
    fetchGeneDetails,
     ...
     fetchClinicalVariants as apiFetchClinicalVariants,
    type GeneBounds, type GeneDetailsFromSearch, 
    type GeneFromSearch, 
    type ClinvarVariant 
...
export default function GeneViewer({
    ...
    const [geneSequence, setGeneSeqeunce] = useState("")
    ...
    const updateClinvarVariants = (clinvar_id: string, updateVariant: ClinvarVariant) => {
    ...
    useEffect(() => {
        const initializeGeneData = async () => {
            ...
            try {
                const {geneDetails: fetchedDetail, geneBounds: fetchedGeneBounds, initialRange: fetchedRange} = await fetchGeneDetails(gene.gene_id)
    ...
    const fetchClinicalVariants = async () => {
        ...
        try {
            const variants = await apiFetchClinicalVariants(
    ...
    <div className="space-y-6">
        ...
        <KnownVariants
            refreshVariants={fetchClinicalVariants}
            showComparison={showComparison}
            updateClinvarVariant={updateClinvarVariants}
            clinvarVariants={clinvarVariants}

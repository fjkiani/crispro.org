...
export async function getAvailableGenomes() {
    ...
    if (!genomeData.ucscGenomes) {
        throw new Error("UCSC API error: missingucscGenomes")
    }
...
export async function fetchGeneDetails(
    ...
    try {
        ...
        if (!detailResponse.ok) {
            console.error(`Failed to fetch gene details: ${detailResponse.statusText}`)
...
export async function fetchGeneSequence(
    ...
    try {
        ...
        return {sequence: "", actualRange: {start, end}, error: "Internal Error in getch gene sequence"}
    }
}
